...
class DBWrapper:
    ...
    async def open_connection(self):
        self._connection = await asyncpg.connect(**self.config)
    ...
    async def create_tables_with_index(self):
        table1 = """CREATE TABLE garages (
                    ...
                    CREATE INDEX _index1_ on garages (id);
                  """
        ...
        table2 = """CREATE TABLE staff (
                  ...
                  CREATE INDEX _index2_ on staff (garage_number, surname);
                  """
    ...
    async def drop_tables(self):
        await self.exec_query('DROP TABLE garages, staff CASCADE;')

    async def fill_with_random(self):
        gen = GenData()
        print('Generate data')
        garages = gen.gen_data_for_garages(200000)
        mechanics = gen.gen_data_for_mechanic(200000)
    ...
    async def make_join(self):
        ...
        query = "SELECT * FROM staff JOIN garages ON garages.id = staff.garage_number WHERE staff.surname LIKE '%son';"
