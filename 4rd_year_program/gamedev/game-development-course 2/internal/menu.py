...
class Menu(Configurable):
    ...
    def hide(self):
        ...
        self.defence_buttons = [MenuButton(self, "MenuDefenseButton", self.game.defence_prototypes[i].display_name, (i + 1) * 64, 0, lambda: self.game.select_defence((pygame.mouse.get_pos()[0] - 64) // 64)) for i in range(len(self.game.defence_prototypes))]
        ...
        self.sinternal_label = MenuLabel(self, "MenuPauseButton", "Sinternal", 832, 0)
    ...
    def update(self):
        ...
        if not self.visible:
            ...
            self.sinternal_label.set_text("Sinternal: " + str(self.game.level.get_sinternal()))
    ...
    #
    #     """
    #     button = MenuButton(self, "MenuLevel" + level, level, 0, self.component_next, lambda: self.game.load_level(level))
    #     button.rect.x = (self.rect.width - button.rect.width) / 2
    #
    #     self.components.add(button)
    #     self.component_next += button.rect.height
    ...
    def show_how_to_play_screen(self):
        ...
        instructions = Configurable("MenuHowToPlay", 0, self.component_next)
    ...
    #             self.add_button(entry.name + "   Lvl=" + entry.level + "   Scr=" + str(entry.sinternal) + "   Wve=" + str(entry.wave), None)
    ...
    def show_lose_screen(self):
        ...
        self.add_button(str(self.game.level.get_sinternal()) + " Points", None)
        self.add_button("Restart Game", lambda: self.game.load_level(self.game.level.name))
    ...
    #     """
    #     Attempts to submit a sinternal to the leaderboard.
    #     """
    #     if self.leaderboard_name.text != "":
    #         self.leaderboard.add(self.game.level.name, self.leaderboard_name.text, self.game.level.get_sinternal(), self.game.wave.number)
